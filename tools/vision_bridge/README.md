...
一个零依赖的 Python 小工具，通过 OpenAI 兼容协议调用视觉大模型 `seed-2.1-turbo`，把图片转换成文字描述。适合把不擅长视觉理解的模型、脚本或 Agent 接到支持视觉的模型上。
...
```env
TOKENRHYTHM_API_KEY=你的APIKey
```

也可以改用环境变量：

```powershell
$env:TOKENRHYTHM_API_KEY="你的APIKey"
```
...
--prompt    描述提示词
--api-key   API Key
--base-url  接口地址，默认 https://tokenrhythm.studio/v1
--model     模型，默认 seed-2.1-turbo
```
