...
> 状态：已实现，模拟器桌面验收通过（2026-08-08），对应 [update-plan.md](update-plan.md) 阶段 1。
...
- Android `DaymarkWidgetProvider` 按样式生成背景位图：卡片主色圆角、贴纸透明、照片图、玻璃半透明白、拍立得白底下沿、霓虹深色描边、像素深底强调描边、极简透明细边框。
- 圆角按小部件实际尺寸收敛：卡片 / 玻璃 / 照片 10dp，霓虹 / 极简 8dp，拍立得 4dp，像素直角，预览与 Android 侧同步。
- 桌面小部件渲染以应用内预览为基准：玻璃 72% 白、像素改为深底强调描边、照片按中心裁切并带圆角、贴纸 / 照片底部渐变压暗与预览同 stops，文字颜色与字号保持一致。
...
- AppWidget 布局只允许系统白名单内的 View 类型，纯 `<View>` 会被 launcher 拒绝膨胀并提示「无法加载微件」，遮罩统一用 `FrameLayout` / `ImageView` 承载。
- 阶段 2-5 的滚动列表、文案、交互与趣味主题不在本阶段实现。

## 验收确认

MuMu 模拟器（x86_64 / Android 12）验收通过，日期 2026-08-08：

- [x] 已安装并启动新构建，桌面小部件正常渲染，无「无法加载微件」提示
- [x] 应用内预览与桌面小部件样式一致：圆角、玻璃透明度、像素描边、照片中心裁切、底部渐变压暗、文字颜色与字号
- [x] 8 套样式预设选择后立即持久化并同步预览与小部件
- [x] 壁纸取色与相册背景写入小部件配置，旧配置缺失字段时回退卡片样式
- [x] 节日皮肤按本地日期计算并显示徽章与强调色
- [x] `flutter analyze`、`flutter test`、Android `testDebugUnitTest` 全部通过
- [x] README、update-plan 与 release notes 同步更新
